...
def real_eval_iemocap(results, truths):
    emos = ["Neutral", "Happy", "Sad", "Angry"]
    ...
    #             max = b - a
    #             pred = i
    #     real_test_preds.append(pred)
    real_test_preds = np.argmax(np.diff(test_preds), axis=1)
    real_test_truth = np.argmax(test_truth, axis=1)

    f1 = f1_score(real_test_truth, real_test_preds, average='weighted')
    wa = accuracy_score(real_test_truth, real_test_preds)
    ua = balanced_accuracy_score(real_test_truth, real_test_preds)
    print("Overall: ")
    print("  - F1 Score: ", f1)
    print("  - Weighted Accuracy: ", wa)
    print("  - Unweighted Accuracy: ", ua)

def real_real_eval_iemocap(results, truths):
    emos = ["Neutral", "Happy", "Sad", "Angry"]
    ...
    wa = accuracy_score(test_truth, test_preds)
    ua = balanced_accuracy_score(test_truth, test_preds)
    print("Overall: ")
    print("  - F1 Score: ", f1)
    print("  - Weighted Accuracy: ", wa)
    print("  - Unweighted Accuracy: ", ua)