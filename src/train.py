...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    if hyp_params.aligned or hyp_params.model=='MULT':
        ...
        ctc_a2l_module, ctc_v2l_module = ctc_a2l_module.to(hyp_params.device), ctc_v2l_module.to(hyp_params.device)
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer, ctc_criterion):
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            text, audio, eval_attr = text.to(hyp_params.device), audio.to(hyp_params.device), eval_attr.to(hyp_params.device)
            ...
            if ctc_criterion is not None:
                ...
                ctc_loss = ctc_loss.to(hyp_params.device)
            else:
                ctc_loss = 0
            ######## CTC ENDS ########
            ...
            if batch_chunk > 1:
                ...
                for i in range(batch_chunk):
                    ...
                    preds_i, hiddens_i = net(text_i, audio_i)
                    
                    if hyp_params.dataset == 'iemocap':
                        preds_i = preds_i.view(-1, 2)
                        eval_attr_i = eval_attr_i.view(-1)
                    print(preds_i, eval_attr_i)
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        if epoch % 5 == 0 or val_loss < best_valid:
            ...
            if hyp_params.dataset == "mosei_senti":
                ...
                # real_real_eval_iemocap(results, truths)
                eval_iemocap(results, truths)
                real_eval_iemocap(results, truths)
    ...
    #
    #
    # print(results, truths)
    # results = np.round(results.view(-1).cpu().detach().numpy())
    # truths = np.round(truths.view(-1).cpu().detach().numpy())
    # print(results, truths)
    #
    # print(confusion_matrix(results, truths))
